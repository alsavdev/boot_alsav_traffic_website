...
const disableimages = document.getElementById('disable_images')
...
document.addEventListener('change', (e) => {
  // if (proxyToggle.checked === true) {
  //     proxyValue.disabled = false
  // }      
...
startButton.addEventListener('click', () => {
  ...
  const vpns = vpn.checked
  const cookiess = cookies.files[0]?.path
  const countrys = country.files[0]?.path
  const disableimagess = disableimages.checked
  
  ipcRenderer.send('button-click', keywordFilePath, googleSearchs, directLinks, visitAdss,  proxyC, proxys, desktops, androids, iphones, randoms, whoers, view, recentPosts, loops, scrollmins, scrollmaxs, scrollminAdss, scrollmaxAdss, captchaApiKeys, ipsayas, anchorTexts, vpns, cookiess, countrys, disableimagess);
});
